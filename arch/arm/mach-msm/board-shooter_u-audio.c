...
void shooter_u_snddev_receiver_pamp_on(int en)
{
	...
	if (en) {
		...
		gpio_direction_output(PM8058_GPIO_PM_TO_SYS(SHOOTER_U_AUD_HANDSET_ENO), 1);
	} else {
		
		gpio_direction_output(PM8058_GPIO_PM_TO_SYS(SHOOTER_U_AUD_HANDSET_ENO), 0);
	}
}
...
void shooter_u_snddev_stereo_mic_pamp_on(int en)
{
	...
	if (en) {
		shooter_u_snddev_imic_pamp_on(en);
		shooter_u_snddev_bmic_pamp_on(en);
	} else {
		shooter_u_snddev_imic_pamp_on(en);
		shooter_u_snddev_bmic_pamp_on(en);
	}
...
int shooter_u_get_speaker_channels(void)
{
...
int shooter_u_support_adie(void)
{
	return 1;
}
...
int shooter_u_is_msm_i2s_master(void)
{
...
void shooter_u_set_q6_effect_mode(int mode)
{
	pr_aud_info("%s: mode %d\n", __func__, mode);
	atomic_set(&q6_effect_mode, mode);
}

int shooter_u_get_q6_effect_mode(void)
{
...
static struct q6v2audio_icodec_ops iops = {
	...
	.support_adie = shooter_u_support_adie,
};
...
static struct acoustic_ops acoustic = {
	...
	.get_speaker_channels = shooter_u_get_speaker_channels,
...
static struct q6asm_ops qops = {
	.get_q6_effect = shooter_u_get_q6_effect_mode,
};
...
void shooter_u_audio_gpios_init(void)
{
	...
	gpio_request(PM8058_GPIO_PM_TO_SYS(SHOOTER_U_AUD_HANDSET_ENO), "AUD_HANDSET_ENO");
}
